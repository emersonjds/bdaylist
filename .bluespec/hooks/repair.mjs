...
var orphanReason = (entry, observed) =>
    observed.some((item) => item.name !== entry.name && samePaths(item.paths, entry.paths))
      ? "renamed-candidate"
      : "orphan",
  repairEntries = (map, observed) => {
    let folded = foldEntries(map, observed),
      observedNames = new Set(observed.map((item) => item.name)),
      unresolved = map.entries
  ...
  repair = async (targetDir, payload) => {
    ...
    if (entries.length === 0)
      throw new Error("repair input needs `entries`, each an item with `name` and `paths`");
    let map = await loadTrackingMap(targetDir),
      result = repairEntries(map, entries);
    return (
